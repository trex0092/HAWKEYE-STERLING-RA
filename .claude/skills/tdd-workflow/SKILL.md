<!--
  Sourced from affaan-m/ECC (https://github.com/affaan-m/ECC), MIT License.
  Copyright (c) 2026 Affaan Mustafa. Reproduced under the MIT License terms;
  see .claude/THIRD_PARTY_LICENSES.md for the full license text.
  Content below is unmodified from the upstream source unless marked
  "HAWKEYE-STERLING-RA note" inline.
-->
...
> **HAWKEYE-STERLING-RA note (added, not upstream):** skip the package-manager
> detector below entirely. This repo does not use npm/pnpm/yarn/bun script-based
> test detection. Its actual commands, from `package.json` as of 2026-09-26, are:
>
> | Purpose | Command |
> |---|---|
> | Unit/integration tests | `node scripts/run-tests.mjs` |
> | E2E tests | `npm run test:e2e` (Playwright, `playwright.cross-browser.config.mjs`) |
> | Visual tests | `npm run test:visual` (Playwright, `playwright.config.mjs`) |
> | Lint | `npm run lint` (eslint against the app's own JS files) |
>
> There is no build step and no runtime npm dependency (per the repo's own
> `package.json` description); devDependencies are dev/CI tooling only
> (`@axe-core/playwright`, `@playwright/test`, `eslint`, `fast-check`, `htmlhint`).
> If this repo's test setup changes, update this note rather than the table below,
> which is upstream ECC content describing the general npm/pnpm/yarn/bun case.
