...
| CA-13 | 2026-07-02 | 42 open code-scanning alerts (CodeQL/Semgrep) surfaced by the daily AI Governance Report but untriaged | Medium | Alert triage cadence not yet established | Triage in GitHub → Security → Code scanning: dismiss false positives with a reason, fix or accept-and-document the rest; add the count to the monthly KPI log | Open-alert count trending down in the daily governance report | Open — owner: maintainer |
