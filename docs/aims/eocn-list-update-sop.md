...
| 2026-09-09 | Compliance Officer (mechanical comparison executed by compliance automation via direct PDF table extraction, from the official publication supplied by the Compliance Officer; the merge of the recording PR is the sign-off) | full reconciliation (§4, both directions) | official EOCN publication supplied as `SEPTEMBER_09__2026.pdf` (12 pages). **Currency caveat**: the file's own embedded metadata shows a 2026-05-12 creation timestamp, matching the source already used for the 2026-08-10 row above — this review is a re-confirmation of no drift as of today, not evidence of a newer designation batch (the automation could not independently re-pull the page itself; uaeiec.gov.ae blocks automated access) | 619 → 619 | Same structure as 2026-08-10: 171 individuals + 75 organisations + 65 entities = **311 listed, every ordinal present**; 19 + 8 = **27 de-listed**. Direction 1: **0 missing**. Direction 2: **0 stale**. Spot-checked (not just aggregate counts) against the same two known tricky multi-name cells (#108 Farhan, #121 Khanfurah) — both still resolve correctly. Incidental, uncorrected observation: `entries` holds two separate objects for the same person in two cases (Hazem Mohsen Farhan; Alaa Khanfurah) — each still reconciles cleanly on its own, so it does not change the result, but is worth a future cleanup pass. | none required; no entry added, changed or removed; `lastReviewed` refreshed to 2026-09-09 | Compliance Officer (PR merge) |
...
2026-09-09. The next is due per the §1 triggers or the weekly review window,
