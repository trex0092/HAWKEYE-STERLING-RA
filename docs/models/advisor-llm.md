...
| **Deployment constraint** | The function must return **inside the platform execution cap** — ~10 s for a synchronous Netlify function by default, raisable per site only by Netlify support. Token budgets are clipped to what that cap affords, and **deep mode is unavailable below 4096 affordable tokens**: it degrades to balanced *visibly* (`modeDegraded` in the response, `modeDegraded=deep→balanced` on the audit line, a banner in the UI) rather than returning a thin answer under the deep label. Raise the site cap and set `ADVISOR_PLATFORM_CAP_MS` to match, and deep mode becomes available with no code change. **Before 2026-07-29 the function aborted at 26 s against a ~10 s cap**, so deep mode was killed by the platform mid-flight — and a killed invocation never returns, so *none* of the guardrails in this card ran. |
