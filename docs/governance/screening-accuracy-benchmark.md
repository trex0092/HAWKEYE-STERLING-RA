...
|---|---|---|
| `recall-pairs.json` | 134 true-equivalent (subject, listed) pairs a competent matcher SHOULD link, labelled by script group and by the engine `mechanism` expected to catch them | sanctions **recall** (per mechanism) |
| `hard-negatives.json` | 90 non-equivalent pairs that must stay clear — boilerplate-only corporates, single-token subsets, single-shared-distinctive-token corporates, phonetic-adjacent distinct persons | hard-negative **clear rate** (precision proxy) |
