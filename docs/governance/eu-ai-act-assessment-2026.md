...
biometric categorisation, no real-time remote biometric ID. Entity-level AML risk
assessment matches none of the Art. 5 categories.
...
Art. 4 (in force 2 Feb 2025) requires providers and deployers to ensure a
sufficient level of AI literacy in staff operating AI systems — it applies to
**every** risk tier, so it is adopted here even while the Act itself does not
bind. The firm's literacy baseline, per role:
...
2. **New AI use case** — especially anything scoring or profiling **natural
   persons** (re-opens Annex III 5(b)).
...
5. **Act evolution** — Commission guidance or delegated acts materially changing
   Annex III or Art. 50 scope (watch via the regulatory-watch pipeline;
   `data/reg-sources.json` sources).
