...
| 2026-08-10 | KRI-10 | `residualAboveAppetite` | 2 vs ≤ 0 | MLRO / DPO — recorded by the merge of the PR that opened the risk | **R-21 (shadow AI)** joins the measure at residual **12 Medium** against RA-04's ceiling of **6**. It is the register's only row whose controls are **policy-only**: the [AUP](ai-acceptable-use-policy.md) now names third-party AI tools and the [asset register](ai-asset-register.md) is the approved list, but there is no managed-device fleet and no DLP behind either, so nothing enforces the rule. The measure went from 1 to 2 the moment the risk was written down — the risk itself is not new, only its record | open |
